...
var scavengeInfo; var tempElementSelection="";

//relocate to mass scavenging page
if (window.location.href.indexOf('screen=place&mode=scavenge_mass') < 0) {    //relocate
...
//set translations
var langShinko = ["Mass scavenging","Select unit types/ORDER to scavenge with (drag units to order)","Select categories to use",
"When do you want your scav runs to return (approximately)?","Runtime here","Calculate runtimes for each page","Creator: ",
"Mass scavenging: send per 50 villages","Launch group "]

if (game_data.locale == "ro_RO") {    //romanian server
langShinko = ["Curatare in masa","Selecteaza tipul unitatii/ORDONEAZA sa curete cu (trage unitatea pentru a ordona)",
"Selecteaza categoria","Cand vrei sa se intoarca trupele de la curatare (aproximativ)",
"Durata aici","Calculeaza durata pentru fiecare pagina","Creator: ","Cueatare in masa: trimite pe 50 de sate","Lanseaza grup "    ]}
if (game_data.locale == "ar_AE") { //arabic server
langShinko=["الاغارات","اختار الوحدات المستخدمة فى الاغارات","اختار انواع   الاغارات المستخدمة ","ما المده المده الزمنيه المراد ارسال الاغارات بها","ضع االمده هنا","حساب المده لكل صفحه ","Creator: ","الاغارات : ترسل لكل 50 قريه على حدى "," تشغيل المجموعة "]
}
if (game_data.locale == "el_GR") {//greek server
langShinko = ["Μαζική σάρωση","Επιλέξτε τις μονάδες με τις οποίες θα κάνετε σάρωση",
"Επιλέξτε επίπεδα σάρωσης που θα χρησιμοποιηθούν","Χρόνος Σάρωσης (Ώρες.Λεπτά)","Χρόνος","Υπολόγισε χρόνους σάρωσης για κάθε σελίδα.",
"Δημιουργός: ","Μαζική σάρωση: Αποστολή ανα 50 χωριά","Αποστολή ομάδας "]}
if (game_data.locale == "nl_NL") { //dutch server
langShinko=["Massa rooftochten","Kies welke troeptypes je wil mee roven, sleep om prioriteit te ordenen",
"Kies categorieën die je wil gebruiken","Wanneer wil je dat je rooftochten terug zijn?",
"Looptijd hier invullen","Bereken rooftochten voor iedere pagina", "Scripter: ","Massa rooftochten: verstuur per 50 dorpen","Verstuur groep " ] }
if ( game_data.locale == "it_IT" ) { //Italian server
langShinko = ["Rovistamento di massa","Seleziona i tipi da unità con cui rovistare","Seleziona quali categorie utilizzare",
"Inserisci la durata voluta dei rovistamenti in ORE","Inserisci qui il tempo","Calcola tempi per tutte le pagine","Creatore: ",
"Rovistamento di massa: manda su 50 villaggi","Lancia gruppo" ] }

//loading settings  // troop types
...
function getData(sendAllDirectly = false) {
 $("#massScavengeSophie").remove();
 URLs = [];
...
<table id="massScavengeSophieFinalTable" class="vis" border="1" style="width: 100%;background-color:${backgroundColor};border-color:${borderColor}">
 ...
 <h3> <center style="margin:10px"><u>
 <font color="${titleColor}">${langShinko[7]}</font> </u>
 </center>
 </h3>
 </td>
 </tr>`;
...
var typnum= ` type="number" step="0.1" `;




//first UI, will always open as soon as you run the script.
var ismobile=document.querySelector('#mobileContent');
var width='600px' , mstyle='' ;
if (ismobile !=null ) {   width='350px';  
mstyle ='<style type="text/css">  *{ padding : 1px !important ;  margin :  1px !important ;} </style>';
  }



html = `
<div id="massScavengeSophie" class="ui-widget-content" style="width:${width};background-color:${backgroundColor};cursor:move;z-index:50;">
<center style="margin-top:1px;">
<input type="button" class="btn btnSophie" id="sendAllMass" onclick="readyToSend(true)" value="ارسال الكل">
</center>
...
</u></center></h3></td></tr><tr id="imgRow">
</tr></table><hr>
<table class="vis" border="1" style="width: 100%;background-color:${backgroundColor};border-color:${borderColor}">
<tbody>
<tr style="background-color:${backgroundColor}">
<td style="text-align:center;background-color:${headerColor}" colspan="4"><h3>
<center style="margin:10px"><u>
...
<td style="text-align:center; width:auto; background-color:${backgroundColor}"><center>
  <input type="checkbox" ID="category1" name="cat1">
</center></td>
<td style="text-align:center; width:auto; background-color:${backgroundColor}"><center>
  <input type="checkbox" ID="category2" name="cat2">
</center></td>
<td style="text-align:center; width:auto; background-color:${backgroundColor}"><center>
  <input type="checkbox" ID="category3" name="cat3">
</center></td>
<td style="text-align:center; width:auto; background-color:${backgroundColor}"><center>
  <input type="checkbox" ID="category4" name="cat4">
</center></td></tr></tbody></table><hr>
<table class="vis" border="1" style="width: 100%;background-color:${backgroundColor};border-color:${borderColor}">
...
<td style="text-align:center; width:auto; background-color:${headerColor};padding: 10px;">
<font color="${titleColor}">Off villages</font></td>
<td style="text-align:center; width:auto; background-color:${headerColor};padding: 10px;">
...
<td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;">
...
<td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;">
  <input type="text" class="runTime_off" ${typnum} style="background-color:${backgroundColor};color:${titleColor};" value="${runTimes['off']}" onclick="this.select();">
</td>
<td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;">
   <input type="text" class="runTime_def" ${typnum} style="background-color:${backgroundColor};color:${titleColor};" value="${runTimes['def']}" onclick="this.select();">
</td></tr><tr>
...
function readyToSend(sendAllDirectly = false) {
...
function sendGroup(groupNr, premiumEnabled) {
 if (premiumEnabled == true) {
  ...
  actuallyEnabled = confirm("Are you sure you want to send the scavenge runs using premium? Cancelling will send the scav run without premium.   ********* DEPENDING ON HOW MANY UNITS/VILLAGES YOU SEND WITH, THIS CAN RESULT IN VERY HIGH AMOUNTS OF PP BEING USED! ONLY USE THIS IF YOU CAN AFFORD IT/KNOW HOW MUCH THE INDIVIDUAL PP RUNS WOULD COST YOU! *********");
    ...
    if (actuallyEnabled == true) { tempSquads = squads_premium[groupNr];  }
    else {
 ...
 TribalWars.post('scavenge_api', { ajaxaction: 'send_squads' }, 
    { "squad_requests": tempSquads }, function () { UI.SuccessMessage("Group sent successfully"); },
        !1
    ); 
...
function sendAllGroups() {
    let groupKeys = Object.keys(squads);
    let i = 0;

    $(':button[id^="sendMass"]').prop('disabled', true);
    $(':button[id^="sendMassPremium"]').prop('disabled', true);
    $('#sendAllMass').prop('disabled', true);

    function sendNext() {
        if (i >= groupKeys.length) {
            UI.SuccessMessage("All groups sent successfully");
            return;
        }

        let g = groupKeys[i];
        if (!squads[g] || squads[g].length === 0) {
            i++;
            sendNext();
            return;
        }
